...
process_execute (const char *file_name)
{
  ...
  for(e = list_begin(&(cur->child_list)); e != list_end(&cur->child_list); e = list_next(e))
  {
    ...
    if(check_thread->load_success == 0)
...
void
stack_construction(char** argv, int argc, void** esp)
{
  int i, j, word_align = 0;
...
start_process (void *file_name_)
{
  ...
  ret_ptr = strtok_r(copy_file_name, " ", &next_ptr);
  while(ret_ptr)
  {
    argc++;
  ...
  strlcpy(copy_file_name, file_name, strlen(file_name) + 1);
  argv[0] = strtok_r(copy_file_name, " ", &next_ptr);
  ...
  if(success)
  {
    stack_construction(argv, argc, &if_.esp);
    // hex_dump(if_.esp, if_.esp, PHYS_BASE - if_.esp, true);
    thread_current()->exit_status = 0;
    sema_up(&(thread_current()->parent_thread->sema_load));
    free(argv);
  }
  /* If load failed, quit. */
...
int process_add_file(struct file * f)
{
  ...
  for(i = 3; i <= cur->file_num; i++)
  {
    ...
    {
      cur->file_descriptor[cur->file_num] = f;
      return i;
    }
  }

  cur->file_num = cur->file_num + 1;
  cur->file_descriptor[cur->file_num] = f;
